...
export default class PreviewDocuments extends Component {
  ...
  getDocuments = () => {
    const { models : {Queue}, id, source } = this.props;
    console.log(id, source);
    this.setState({ loading : true });
    
    fetch(`${source}/api/queues/${id}?q=eyJpbmNsdWRlIjpbeyJtb2RlbCI6IkRvY3VtZW50IiwiYXR0cmlidXRlcyI6WyJuYW1lIiwiaWQiLCJkYXRhIl19XSwiYXR0cmlidXRlcyI6WyJpZCIsIm5hbWUiLCJpZCJdfQ==`)
      .then((res) => res.json()).then((res) => res.data).then(({ name, documents }) => this.setState({name, documents, loading: false})).catch(e => console.log(e));
    // Queue.single(id, {
    //   include : [{
    //     model : 'Document',
